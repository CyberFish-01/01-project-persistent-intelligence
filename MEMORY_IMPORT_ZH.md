...
`import-text` 现在会把每次运行视为一个 import batch。

写入 `imports.jsonl` 的报告包含：
...
导入器会在当前批次内去重，也会和已经存在的 `imported_memory` 去重。

它还会保守过滤明显敏感材料，例如 password、token、API key、secret、private key，以及中文的密码、口令、密钥、令牌、私钥等标记。被过滤的敏感内容不会作为 imported memory 存储，也不会以明文或 hash 复制进报告；敏感项报告只保留 source index 和 reason。

如果所有候选都被跳过，`import-text` 仍会写入 import report 和 audit/trace 事件，但不会创建 Dream job。

这些检查不能替代人工 review。它们只是进入 Dream review 之前的第一道安全边界。

## 7. 为什么不直接写入 Identity Core
...
6. 运行 `dream`；
7. 检查 Dream 是否提出合适的 candidate memory；
8. 人工批准后用 `promote-candidate` 提升到 active semantic memory；
9. identity update 仍必须通过 high gate，不能自动批准。
