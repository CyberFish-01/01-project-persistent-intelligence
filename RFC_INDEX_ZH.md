...
| P134 | [FOUNDER_CONSOLE_ACCEPTANCE_CRITERIA.md](./FOUNDER_CONSOLE_ACCEPTANCE_CRITERIA.md) / [ZH](./FOUNDER_CONSOLE_ACCEPTANCE_CRITERIA_ZH.md) | acceptance criteria | document-only gate | 定义 future founder console 在 implementation 被认为可接受前必须通过或失败的条件。 | console implementation、Web UI、Companion、adapter integration、model call、tool execution、write path、policy executor 或 rebuild |
...
| P139 | [SOURCE_SELECTION_MATRIX.md](./SOURCE_SELECTION_MATRIX.md) / [ZH](./SOURCE_SELECTION_MATRIX_ZH.md) | matrix | document-only selection matrix | 定义 pack-level source preferences、trust levels、omission reasons，以及 temporal/capability selection boundaries。 | retrieval implementation、ranking engine、builder、model call、prompt execution、state write、memory write、adapter integration、tool execution 或 rebuild |
| P140 | [BOUNDARY_INJECTION_RFC.md](./BOUNDARY_INJECTION_RFC.md) / [ZH](./BOUNDARY_INJECTION_RFC_ZH.md) | boundary RFC | RFC-only injection contract | 定义 future context packages 如何在 boundary 和 response-strategy packs 中携带 boundary reminders。 | builder implementation、prompt builder、runtime guard、model call、policy executor、state write、memory write、adapter integration、tool execution 或 rebuild |
...
| P145 | [POST_RESPONSE_CANDIDATE_EXTRACTION_RFC.md](./POST_RESPONSE_CANDIDATE_EXTRACTION_RFC.md) / [ZH](./POST_RESPONSE_CANDIDATE_EXTRACTION_RFC_ZH.md) | extraction RFC | RFC-only extraction boundary | 定义 future model output 如何被检查成 preview-only candidates，并带有 review gates 和 blocked promotions。 | extraction runtime、model call、response generation、candidate storage、review lifecycle、event write、memory write、identity mutation、tool execution 或 rebuild |
...
39. [BASELINE_TAGGING_FOUNDER_REVIEW.md](./BASELINE_TAGGING_FOUNDER_REVIEW.md)
    和 [BASELINE_TAGGING_FOUNDER_REVIEW_ZH.md](./BASELINE_TAGGING_FOUNDER_REVIEW_ZH.md)
    记录 P157 founder review：recommendations 更清楚，但 tag 和 branch execution 仍等待 manual
