...
P28 adds tool/safety proposal link lifecycle retention. `tool-safety-policy-link-lifecycle` can archive, discard, or quarantine reviewed proposal links. It writes snapshot, audit, trace, update log, lifecycle history, and `task_hub.tool_safety_policy_link_lifecycle_decisions`, while preserving `relationship_mode: "review_link_only"` and non-executable invariants. Context packages expose only active proposal links, so archived, discarded, and quarantined links are suppressed from active state transfer.
...
P43 adds a read-only evidence coverage mapping:

```bash
python3 -m one_core.cli reconstruction-evidence-coverage-map
```

This report maps current event workflows to the P42 evidence sections so the project can see which workflow families need payload, diff, snapshot, or seed/pre-event references before any schema mutation.
...
P47 adds a read-only reconstruction schema review coverage map:

```bash
python3 -m one_core.cli reconstruction-schema-review-coverage-map
```

This report maps P46 decisions back to P45 checklist items so the project can see which workflow gaps have been reviewed and which remain unreviewed.
...
P47 does not approve schema changes, capture payloads, reconstruct state, compact events, roll back state, mutate identity, or modify the event log. It only exposes governance coverage before any schema mutation or reconstruction work exists.

P48 adds a read-only reconstruction schema review evidence request tracker:

```bash
python3 -m one_core.cli reconstruction-schema-review-evidence-requests
```

This tracker derives open evidence requests from P46 review decisions and P47 coverage. It gives each request a stable derived id so the same state can be inspected repeatedly without creating new records.
