...
P95 RFC-only plan，规划未来可能存在的 read-only command，用 approved documents 和 static
state summaries 生成 Foundation Observatory report。

边界：该 plan 不实现 CLI commands、parsers、generators、dashboards、Web UI、product UI、status
APIs、runtime monitors 或 executors。

## Observatory CLI Report / 观察台 CLI 报告

未来可能由 minimal observatory command 输出的 read-only report，包含 founder snapshot、axes map、
...
边界：P95 明确阻止 observability execution。除非未来 founder-approved phase 另行定义，
