...
static CLI report generator。

边界：P96 仍只是 static report generator。它不实现 dashboard runtime、Web UI、status API、
...
P96 read-only CLI command：

```bash
python3 -m one_core.cli foundation-observatory-report
```

它使用 static foundation artifacts 和 approved naming/status categories 输出 Markdown 或 JSON
founder-facing observatory report。
...
由 P96 minimal observatory command 输出的 read-only report，包含 founder snapshot、axes map、
readiness matrix、boundary status、risk heatmap、next-step recommendations 和 blocked-work list。
...
边界：P96 仍明确阻止 observability execution。除非未来 founder-approved phase 另行定义，
