...
当前工作状态：P123 Shadow Adapter Mode RFC 已完成。项目现在有一份 RFC-only 边界，用于未来
adapter-shaped input：adapter 可以在 shadow mode 中被观察，但不 ingest、不写 event、不拥有 identity、
不写 memory、不调用模型、不接 AstrBot，也不批准 integration。
...
- [PHASE_INDEX.md](./PHASE_INDEX.md) / [PHASE_INDEX_ZH.md](./PHASE_INDEX_ZH.md)：P0-P123 foundation phase index，按核心命题和所属主线整理。
