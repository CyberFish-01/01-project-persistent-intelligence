...
Current work status: P107 Harness Review Queue Preview Specialization is
complete. The local read-only `harness-dry-run` command now gives each review
gate a queue intent, gate-selection reason, blocked-lifecycle reason, and
manual-review-only next action while staying preview-only.
...
- [PHASE_INDEX.md](./PHASE_INDEX.md) / [PHASE_INDEX_ZH.md](./PHASE_INDEX_ZH.md): P0-P107 foundation phase index by proposition and main line.
