...
Current work status: P106 Harness Candidate Preview Specialization is complete.
The local read-only `harness-dry-run` command now gives each candidate a
pressure-specific intent, selection reason, blocked-promotion reason, and manual
review target while staying preview-only.
...
- [PHASE_INDEX.md](./PHASE_INDEX.md) / [PHASE_INDEX_ZH.md](./PHASE_INDEX_ZH.md): P0-P106 foundation phase index by proposition and main line.
